...
**Maestro or Appium cannot start the test runner.** Read the log of the `xcodebuild` process: `xctest_runner_*.log` for Maestro, or the Appium server log with `appium:showXcodeLog: true`. The shim writes its steps there. See [Maestro and Appium shims](/docs/cloud/reference/test-drivers).
